...
| `options.mjs` | 960x540 | ~2 min | every Options entry changed with real keys and measured live in the game (SIGNAL: a dormant Tethered reads in CLASSIC, not in UNRELIABLE), then saved to localStorage and restored after a reload; the calibration screen |
| `signal.mjs` | 960x540 | ~2 min | the unreliable signal: every monster type dormant reads nothing, aware it is the source; the lag, the wobble, 5 bars only within 3 m, the tells on the lagged reading; phantoms (sped-up `Phone.TUNE`) gated, never under overrides / scenes / inserts / menus / an aware threat, seeded, and the real 50–140 s / 35–90 s intervals; no always-on HUD (shown while glancing, under overrides and calls; the phone menu and the in-hand screen show the reading); the hold-C glance with real keys and a pad (tap = the menu on release; the pose, half speed, no running, the torch's pool; never while readied / swinging / crawling / climbing / grabbed); the one-time "Hold C" prompt; CLASSIC equal to the old radar tick for tick, its HUD as before; the SIGNAL option. `SIG_ONLY=C,D,F` runs some sections |
