...
| `signal.mjs` | 960x540 | ~6 min | the unreliable signal: every monster type dormant reads nothing, aware it is the source; the lag, the wobble, 5 bars only within 3 m, the tells on the lagged reading; phantoms (sped-up `Phone.TUNE`) gated, never under overrides / scenes / inserts / menus / an aware threat, seeded, and the real 50–140 s / 35–90 s intervals; CLASSIC equal to the old radar tick for tick; the SIGNAL option |
