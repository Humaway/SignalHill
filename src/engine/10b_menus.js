...
#ui .mn-h{font-size:clamp(13px,1.6vh,20px);letter-spacing:.34em;color:${COL.dim};text-transform:uppercase;white-space:nowrap;}
#ui .mn-legend{position:absolute;left:0;right:0;bottom:3.3vh;text-align:center;font-size:clamp(12px,1.45vh,17px);letter-spacing:.26em;color:#66655f;text-transform:uppercase;white-space:pre;text-shadow:0 0 .4em rgba(0,0,0,.9);}
...
#ui .tt-press{position:absolute;left:0;right:0;top:49%;text-align:center;font-size:clamp(13px,1.7vh,21px);letter-spacing:.44em;padding-left:.44em;color:#bfbbb0;text-shadow:0 0 .45em rgba(0,0,0,.75),0 0 1.3em rgba(0,0,0,.5);}
...
#ui .ps-area{position:absolute;right:4.2vw;top:5.2vh;font-size:clamp(12px,1.5vh,19px);letter-spacing:.3em;color:${COL.title};text-shadow:${SH_};white-space:nowrap;}
...
#ui .it-word{font-size:clamp(12px,1.45vh,18px);letter-spacing:.34em;}
#ui .it-eq{position:absolute;right:4.4vw;top:19.5vh;width:17vw;text-align:center;}
#ui .it-eq-n{font-size:clamp(12px,1.45vh,18px);letter-spacing:.28em;color:${COL.title};white-space:nowrap;}
...
#ui .it-comb{position:absolute;left:0;right:0;top:67.5vh;text-align:center;font-size:clamp(12px,1.5vh,19px);letter-spacing:.34em;color:#a8a59b;visibility:hidden;opacity:0;}
...
#ui .mm-list .mn-it.mm-g{font-size:clamp(12px,1.45vh,18px);letter-spacing:.36em;color:#66655f;padding:2.2vh 0 .5vh;text-shadow:none;}
...
/* doc */
#ui .dc-page{position:absolute;left:0;right:0;bottom:2.2vh;text-align:center;font-size:clamp(12px,1.4vh,17px);letter-spacing:.3em;color:#66655f;}
/* phone */
...
#ui .sv-a{display:block;font-size:.86em;letter-spacing:.2em;color:${COL.faint};margin-top:.35em}
...
/* death */
#ui .dt-ns{position:absolute;left:0;right:0;top:50%;transform:translateY(-50%);text-align:center;font-size:clamp(12px,1.6vh,20px);letter-spacing:.42em;padding-left:.42em;color:${COL.title};}
...
#ui .cr-head{font-size:clamp(12px,1.45vh,18px);letter-spacing:.38em;padding-left:.38em;color:${COL.dim};margin:5vh 0 1.4vh;text-transform:uppercase;}
...
#ui .cr-pair span:first-child{flex:1;text-align:right;color:${COL.dim};font-size:.86em;letter-spacing:.26em;}
...
`;
  ...
  SCREENS.title = {
    ...
    update(sc, dt) {
      ...
      else if (st.stage === 'press' && st.t >= st.at && !st.pressF.busy) st.press.style.opacity = (0.78 + 0.22 * Math.cos((st.t - st.at) * 1.9)).toFixed(3);   // a slow pulse, never below 56 %
      if (st.ringCutAt != null && (!Snd.ctx || Snd.ctx.currentTime >= st.ringCutAt)) { titleRingStop(st, 0); st.ringCutAt = null; }
  ...
  function titleRingStop(st, fade) { st.ringCutAt = null; if (st.ring) { st.ring.stop(fade); st.ring = null; } }
  // §2A step 4: "a distant phone that's been ringing the whole time stops mid-ring". The ring's cadence (every 3 s): a
  // burst 0–0.4 s, a gap, a burst 0.6–1.0 s, then 2 s of quiet — a key pressed in the quiet would stop nothing you can
  // hear, so the ring is cut dead inside a burst: at once when one is sounding, else 0.2 s into the next (≤ 2.3 s on).
  function titleRingCut(st) {
    const r = st.ring, ctx = Snd.ctx;
    if (!r || !ctx || !Number.isFinite(r.t0)) { titleRingStop(st, 0); return; }
    const ph = (((ctx.currentTime - r.t0) % 3) + 3) % 3;
    if ((ph >= 0.08 && ph <= 0.34) || (ph >= 0.68 && ph <= 0.94)) { titleRingStop(st, 0); return; }
    const cut = [0.2, 0.8, 3.2].find((c) => c > ph);
    st.ringCutAt = ctx.currentTime + (cut - ph);
  }
  function titleToMenu(sc) {
    ...
    if (st.ring) { if (st.ringCutAt == null) titleRingCut(st); st.ringDone = true; }
  ...
  SCREENS.map = {
    ...
    build(sc) {
      ...
      // zoomed in, the paper fills the window: the floor tabs and the legend would sit on its printing, so they fade out
      // 2.5 s after the last zoom / floor change and come back at the whole-sheet view
      st.chromeT = 0;
      st.chromeF = new Tw((v) => { for (const e of [st.floorsEl, st.legendEl]) if (e) { e.style.opacity = v.toFixed(3); e.style.visibility = v <= 0.01 ? 'hidden' : ''; } }, 1);
      mapResize(sc);
    },
    ...
    update(sc, dt) {
      ...
      st.chromeT += dt;
      const chrome = !st.zoom || st.chromeT < 2.5 ? 1 : 0;
      if (st.chromeF.target !== chrome) st.chromeF.to(chrome, chrome ? 0.25 : 0.6);
